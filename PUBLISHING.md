...
7.8.4新增[文章管理](ARTICLE-MANAGEMENT.md)：查看发布状态、隐藏、恢复公开及只删除线上文章，后续检查与部署自动完成。

本定制版 7.8.4 基于上游稳定版 7.8.2，并保留上游 AGPL-3.0 许可证和作者归属。它不是上游的新稳定版本。master 保留上游同步历史，定制代码在 codex/publish-center 分支；上游 8.x 测试版不混入此稳定分支。
...
从本fork的7.8.4发行附件取得main.js、manifest.json、styles.css，关闭插件后替换对应插件目录内这三个文件，再启用。保留data.json和原钥匙串，不复制另一台设备的Token。原插件ID不变，因此沿用已有设置。
...
20组发布状态及API交互测试覆盖精确版本、失败和取消、旧检查、不同申请、网络/权限错误、公开链接安全、祖先版本验证、重试目标及旧申请同步。原英文申请标题直接依据7.8.2发行资源，中文资源来自已验证的完整翻译。另修复上游复用已有申请时取错其他PR的问题，严格按owner/repo/head/base匹配。

2026-09-11已完成本地类型检查、构建、20组测试，以及真实Obsidian启动、发布面板和缺权限状态验证。使用原插件Token补充Actions权限后，真实重试、旧申请同步、云端重新检查/自动合并/部署，以及面板核对公开版本并显示「已上线」和文章链接均通过。GET进度请求带刷新参数，避免复用旧状态；不添加会被GitHub跨域限制拦截的自定义请求头。未在另一台设备或移动端验收；测试夹具不等于真实上传或部署。
